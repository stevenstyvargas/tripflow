...
| Sidebar / nav item | default / hover / activo / próximamente | `components/sidebar.js`, fondo navy — solo navegación (Inicio/Mis viajes/Alertas), la cuenta vive en el menú de cuenta. Activo = franja verde (`--color-accent`) pegada al borde izquierdo + texto blanco/bold (ya no fondo, navy sobre navy no contrasta); hover = overlay blanco sutil. El ícono de "Alertas" lleva un `.nav-badge` (círculo rojo, número de alertas activas o "9+", oculto si es 0) — mismo en sidebar desktop y drawer mobile, cuenta viene de `getTripAlerts()` (`data/store.js`), compartida con la pantalla de Alertas. Debajo de "Alertas", 2 ítems "Próximamente" (Mis facturas, Calendario): `<button>` en vez de `<a>` (no navegan), ícono+texto a 55% de opacidad, etiqueta "Próximamente" verde debajo del texto sin atenuar, click dispara `components/toast.js` en vez de navegar — nunca llevan `.is-active` |
...
| Campo de búsqueda | default / focus | `components/search-field.js`, ícono de lupa + input; filtra viajes por nombre en vivo. Usado en Inicio y Mis viajes. En mobile (≤45rem) el input inline de cada página se oculta y se reemplaza por la lupa + fila expandible del header compartido (`components/sidebar.js`) — mismo `<input>`, cada página bindea su filtro a los dos |
...
| Gasto por categoría | con datos / vacío | `components/category-chart.js` — torres (`components/bar-chart.js`, SVG-free, barras CSS) en desktop, dona (`components/donut-chart.js`, SVG puro) en mobile (≤45rem); unificado por BREAKPOINT, no por pantalla — mismo componente en Inicio (agregado de todos los viajes activos) y Detalle de viaje (un solo viaje). La dona centra el total gastado con leyenda de %/monto por categoría debajo; las torres muestran monto arriba + ícono/nombre debajo, color solo en las categorías con gasto (gris en las de $0) |
| Alert item | warning / danger | Borde completo (1px) del color de severidad |
| FAB "Nuevo viaje" | default / hover | `.fab`, solo en Inicio en mobile (≤45rem) — reemplaza al botón del header en ese breakpoint. Circular, fondo `--color-accent` (verde) con ícono navy (blanco da ~1.75:1 de contraste sobre este verde, navy da ~9:1), fixed en la esquina inferior derecha con margen de `env(safe-area-inset-bottom)` |
| Toast | oculto / visible | `components/toast.js`, píldora navy centrada abajo (`position:fixed`), se auto-cierra sola a los 2.5s — no bloquea como `confirm-modal.js`, no requiere respuesta. Usado hoy por los ítems de nav "Próximamente" |
