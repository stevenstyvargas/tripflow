...
| Sidebar / nav item | default / hover / activo | `components/sidebar.js`, fondo navy — solo navegación (Inicio/Mis viajes/Alertas), la cuenta vive en el menú de cuenta. Activo = franja verde (`--color-accent`) pegada al borde izquierdo + texto blanco/bold (ya no fondo, navy sobre navy no contrasta); hover = overlay blanco sutil. El ícono de "Alertas" lleva un `.nav-badge` (círculo rojo, número de alertas activas o "9+", oculto si es 0) — mismo en sidebar desktop y drawer mobile, cuenta viene de `getTripAlerts()` (`data/store.js`), compartida con la pantalla de Alertas |
